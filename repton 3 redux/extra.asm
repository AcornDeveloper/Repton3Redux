...
.all_edge_tiles
 ...
 STZ paras + &00                        ;multiply by sprite size of &80
 LSR A
 ROR paras
 CLC
 ...
 AND #&02
 TAX
 JMP (sides_update,X)                   ;vector to routine to transfer sprite data to buffer
...
.sides_update
 EQUW repton_going_left_right           ;0/1
 EQUW repton_going_up_down              ;2/3
...
_ORA_X                                  ;perform a or x from identity table, slightly faster
 ...
 STZ paras + &05                        ;now transfer buffer to screen one row, eight cells at a time
...
.sprite_mask_foreground                 ;mask sprite onto buffer
 ...
 ASL A                                  ;paras + &01 direction of travel 0-3
 TAX                                    ;paras + &02 counter into travel 0-3
 JMP (buffer_mask_fill,X)               ;jump to routine to mask sprite onto buffer
...
.sprite_column
 LDA (paras + &05)
 STA (paras + &07)
 LDY #&01                               ;transfer one cell of buffer to screen
 LDA (paras + &05),Y
 STA (paras + &07),Y
 INY
 LDA (paras + &05),Y
...
.update_repton_coordinates              ;calculate coordinates as if repton had moved one cell
 ...
 LDA movie
 PHA
 INA                                    ;one space on for new sprite window
 STA movie
 CMP #&04
...
.eor_sprite_loop
 ...
 LDY #&01                               ;eor a sprite cell with the buffer
 LDA (paras + &05)
 EOR (paras + &07)
 STA (paras + &07)
 LDA (paras + &05),Y
