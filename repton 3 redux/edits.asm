...
.repton_not_adjust
 ...
 SEI
 JMP (update_edge_part,X)               ;vector to update screen from buffer routine in direction of travel

.update_edge_part                       ;transfer buffer to screen memory
 EQUW repton_column_transfer
 EQUW repton_column_transfer
 EQUW repton_row_transfer
 EQUW repton_row_transfer
...
.repton_interrupt_start
 STZ scan_line_area                     ;reset area raster beam in
...
.standard_map_file
 STZ map_file_index                     ;point at default character set
 LDA #LO(squeze)
 STA map_file_in_use
 LDA #HI(squeze)
 STA map_file_in_use + &01
